...
public class DlgAdicionarBebida extends JDialog {
	...
	private class ActionOk implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (testaMenorZero(textQuantidade, "quantidade " )) {
				return;
			}
	...
	private boolean testaMenorZero(JTextField textQuantidade, String label) {
		int quantidade = Integer.parseInt(textQuantidade.getText().trim());
		
		if (quantidade <= 0) {
			JOptionPane.showMessageDialog(botaoOk, "Precisa ser um valor maior que zero na quantidade!");
			return true;
		}
		return false;
	}
