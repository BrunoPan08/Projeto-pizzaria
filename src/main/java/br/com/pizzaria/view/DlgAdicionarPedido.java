...
public class DlgAdicionarPedido extends JDialog {
	...
	private int CLIENTE_00(String string) {
	...
	private class ActionOk implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			int clienteVazio = comboCliente.getSelectedIndex();
			int tabelaVazioPizza = pizzaGrid.getRowCount();
			int tabelaVazioBebida = bebidaGrid.getRowCount();
			
			if (clienteVazio == CLIENTE_00("")) {
				JOptionPane.showMessageDialog(botaoOk, "É necessario escolher o nome do cliente!");
				return;
			}
			
			if (tabelaVazioPizza == 0) {
				JOptionPane.showMessageDialog(botaoOk,"É necessario adicionar uma pizza antes!");
				return;
			}
			
			if (tabelaVazioBebida == 0) {
				JOptionPane.showMessageDialog(botaoOk,"É necessario adicionar uma bebida antes!");
				return;
			}
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if (linhaSelecionada < 0) {
					JOptionPane.showMessageDialog(botaoRemoverPizza, "Deve selecionar uma pizza para remover!");
					return;
				}
	...
	private class ActionRemoverBebida implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if (linhaSelecionada < 0) {
					JOptionPane.showMessageDialog(botaoRemoverBebida, "Deve selecionar uma bebida para remover!");
					return;
				}
				
				PedidoBebida pedidoBebida = pedido.getPedidoBebidas().get(linhaSelecionada);
				
				business.removePedidoBebida(pedido, pedidoBebida);
				bebidaTableModel.fireTableDataChanged();
				DlgAdicionarPedido.this.atualizaValorTotal();
			} catch (Exception ex) {
				ex.printStackTrace();
	...
	private class ActionDetalhes implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			try {
				...
				int linhaSelecionada = pizzaGrid.getSelectedRow();
				Pizza pedidoPizza = Pizza.valueOf(getName());
				
				if (linhaSelecionada < 0) {
					JOptionPane.showMessageDialog(botaoDetalhesPizza, "Nenhum item do pedido da Pizza foi selecionada!");
					return;					
				}
				
				if (linhaSelecionada == 1 ) {
					JOptionPane.showMessageDialog(botaoDetalhesPizza, pedidoPizza.getIngredientes());
				}
